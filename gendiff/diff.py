...
def generate_diff(first, second):  # input two parsed data, output AST
    same_key_items = {
        key: {'type': SAME, 'value': first[key]}
        for key in first
        if key in second}
    new_key_items = {
        key: {'type': NEW, 'value': second[key]}
        for key in second
        if key not in first}
    lost_key_items = {
        key: {'type': LOST, 'value': first[key]}
        for key in first
    ...
    for key, value in item.items():
        if isinstance(value, dict):
            if value.get('type') == NESTED:
                ...
                result.append('{}{} {}: {}'.format(
                    indent, value['type'], key, get_value(value, level)))
    if level == 1:
        result.append('}')
    return '\n'.join(result)
...
def get_value(value, level=1):
    ...
    if isinstance(value, dict) and 'type' not in value:
        ...
        for k, v in value.items():
            res += ('{}{}: {}\n'.format(2 * indent, k, v))
        if level >= 2:
            res += ('{}}}'.format(indent + INDENT * (level - 2)))
        else:
            res += ('{}}}'.format(indent))
